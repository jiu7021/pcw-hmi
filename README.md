...
- **판형 열교환기**: Effectiveness-NTU법(Incropera, *Fundamentals of Heat and
  Mass Transfer*, Cmin/Cmax→0 극한 `ε = 1 - exp(-NTU)`)으로 공급온도 산출.
  UA는 유량^0.8에 비례(난류 대류열전달계수 Nu∝Re^0.8 상관식 근사)
- **펌프**: 원심펌프 2차곡선 근사 `H = n²H0 - kQ²` (affinity law) + 헤더
  압력을 이분법(bisection)으로 풀어 3대 병렬 유량 배분을 계산
...
| 플랜트 | 실물 배관·펌프·열교환기 | 수치 모델(1차 지연, effectiveness-NTU, 펌프 2차곡선 등 근사식) |
...
- 배관 마찰손실, 열교환기 파울링, 펌프 캐비테이션, 다중 열교환기 구성 등은
  모델링하지 않았습니다.
