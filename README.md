...
- **고속 샘플링**: 5ms(200Hz, 제어주기의 1/20). 파형 시뮬레이션은 하지
  않고(범위 밖), 전기 계층이 100ms마다 계산하는 모선전압을 "목표치"로 두고
  전기적 시정수(대표값 10ms)로 수렴해가는 과정 자체를 5ms 간격으로 추적해
  "제어 주기보다 훨씬 짧게"라는 요구를 만족시킵니다.
...
tests/
  ...
  scenarios.js       시나리오 A~F 정의 + 게인 배치 시험용 게인 4세트 (SimCore만 사용)
...
않는다. 참값과 측정값이 1.891°C까지 벌어진 상태로 관측 구간(1시간) 내내
...
| 케이스 | 최저 모선전압 | 관리한계(85%) 위반 |
|---|---|---|
| 정상: 바이패스 1대만 기동(나머지 VFD 정상운전) | 89.54% | 아니오 |
| 우회: 바이패스 2대가 동시에 기동 | 83.57% | **예** |
...
실제로 관리한계를 넘는다"는 것을 84.54%→83.57%처럼 애매하게가 아니라
**한쪽은 한계 안(89.54%), 한쪽은 한계 밖(83.57%)** 으로 명확히 갈라
보여준다는 점에서, [수정 1]에서 다시 정의한 "바이패스(DOL) 동시기동
금지"라는 인터록 근거와 재역산한 모선 임피던스 상수(`BUS_SOURCE_
REACTANCE_PU`)가 타당함을 뒷받침하는 수치적 증거로 삼는다.
