...
이 확장 작업은 에이전트형 AI 코딩 도구인 **Claude Code**로 진행했습니다.
"근거 없는 상수는 쓰지 않는다", "기존에 통과하던 검증은 절대 깨뜨리지
않는다", "예상과 다른 결과가 나와도 유리하게 고치지 말고 원인을 찾아
그대로 보고한다" 같은 규칙을 대화 중 계속 적용했고, 그중 언어 규칙 등
일부는 프로젝트 규칙 파일([CLAUDE.md](CLAUDE.md))로 남겨 이후 작업에도
이어지도록 했습니다.
